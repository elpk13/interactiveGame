...
rabbitA = (random.randint(0,worldx),random.randint(0,worldy))
...
def findPath():
    ...
    # rabbitPath[0] = playerx
    # rabbitPath[1] = playery
    ind = 0
    rabRad = 10000
    global rad
    rad.append(5)
    ...
    while rabRad > 400: 
        ...
        if(rabbitA[0] > rabbitPath[-2] and rabbitA[1] >= rabbitPath[-1]):
            ...
            xc = 1
            yc = -1
        rabRadSq = (xLen**2) + (yLen**2)
        rabRad = math.sqrt(rabRadSq)
        theta = math.atan(oppAdj)
        angle = random.randint(0, int(theta * 1000)) / 1000
        rabbitPath.append(rad[-1] * math.cos(angle) * xc + rabbitPath[ind])
        rabbitPath.append(rad[-1] * math.sin(angle) * yc + rabbitPath[ind+1])
        ind += 2
        radInd += 1
        rad.append(random.randint(50, 400)) #calcualte the radius randomly between 25-400
    ...
    # while i < (ind): ## TODO : is the +1 necessary?
    #     numPrint = rad[radInd] / 50
    #     printInd = 1
    #     while printInd < (numPrint):
    #         nr = rad[radInd] - (50 * (printInd))
    #         stf = rad[radInd] / nr
    #         newy.append(int(rabbitPath[i + 1]/stf))
    #         newx.append(int(rabbitPath[i]/stf))
    #         # screen.blit(rabbitImage, (newx[-1],newy[-1]))
    #         # pygame.display.update()
    #         printInd += 1
